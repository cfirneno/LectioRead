...
for (const sc of SCENES) {
  ...
  // Resumable: skip scenes already generated (non-trivial file present).
  if (fs.existsSync(mp3Path) && fs.statSync(mp3Path).size > 2000) {
    const dur = parseFloat(
      execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${mp3Path}"`).toString().trim()
    );
    console.log(`Skipping ${sc.key} (exists, ${dur.toFixed(2)}s)`);
  ...
  // Atomic write so a timeout mid-scene never leaves a truncated mp3.
  const tmpPath = mp3Path + '.tmp';
  fs.writeFileSync(tmpPath, Buffer.from(audio.data, 'base64'));
