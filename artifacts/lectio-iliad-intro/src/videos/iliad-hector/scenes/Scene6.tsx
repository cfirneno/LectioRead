  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        <motion.img
          src={`${import.meta.env.BASE_URL}images/iliad-hector/zeus-scales.png`}
          alt="Zeus scales"
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-12 max-w-5xl">
        ...
        >
          "High on Olympus, Zeus lifts his golden scales<br/>and weighs two fates of death."
        </motion.div>
        
        <motion.div 
          className="text-[3vw] font-display text-secondary/90 italic drop-shadow-2xl"
          initial={{ opacity: 0, filter: 'blur(10px)' }}
          animate={phase >= 2 ? { opacity: 1, filter: 'blur(0px)' } : { opacity: 0, filter: 'blur(10px)' }}
          transition={{ duration: 2, ease: "easeOut" }}
        >
          "The doom of Hector sinks down toward the house of Hades."
        </motion.div>
      </div>
    </motion.div>
  );
